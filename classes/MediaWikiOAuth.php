...
class MediaWikiOAuth
{
    ...
    function __construct($t, $l = '', $p = '', $oauth_url = '')
    {
        if (is_array($t)) { // Bespoke override for third-party sites
            ...
            if ($l == 'wikidata') {
                ...
                $this->apiUrl = 'https://www.mediawiki.org/w/api.php';
            } else {
                $this->apiUrl = "https://$l.$p.org/w/api.php";
            }
        }
    ...
    function sleepAfterEdit($type)
    {
        ...
        if ($type == 'upload') {
            sleep($this->delay_after_upload_s);
        }
    }
    ...
    function loadIniFile()
    {
        ...
        if (!isset($this->gUserAgent) or $this->gUserAgent == '') {
            throw new Exception ("Cannot get user agent from ini file '{$this->ini_file}'");
        }
        $this->gConsumerKey = $this->params['consumerKey'];
        if (!isset($this->gConsumerKey) or $this->gConsumerKey == '') {
            throw new Exception ("Cannot get consumer key from ini file '{$this->ini_file}'");
        }
        $this->gConsumerSecret = $this->params['consumerSecret'];
        if (!isset($this->gConsumerSecret) or $this->gConsumerSecret == '') {
            throw new Exception ("Cannot get consumer secret from ini file '{$this->ini_file}'");
        }
    }
    ...
    function fetchAccessToken()
    {
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, env('CURLOPT_SSL_VERIFYPEER', true, 'bool'));
    ...
    function doApiQuery($post, &$ch = null, $mode = '', $iterations_left = 5, $last_maxlag = -1)
    {
        if ($iterations_left <= 0) {
            return;
        } // Avoid infinite recursion when Wikidata Is Too Damn Slow Again
        ...
        if (isset ($_REQUEST['test'])) {
            print "<pre>GIVEN MAXLAG:{$give_maxlag}</pre>";
        }
        ...
        if (isset($ret->error) and isset($ret->error->code) and $ret->error->code == 'maxlag') {
            $lag = $maxlag * 1;
            if (isset($ret->error->lag)) {
                $last_maxlag = $ret->error->lag * 1 + $maxlag * 1;
            }
            sleep($lag);
            $ch = null;
    ...
    function doesClaimExist($claim)
    {
        ...
        if (!isset ($j->entities->$q)) {
            return false;
        }
        if (!isset ($j->entities->$q->claims)) {
            return false;
        }
        if (!isset ($j->entities->$q->claims->$p)) {
            return false;
        }
        ...
        foreach ($cp as $k => $v) {
            if ($claim['type'] == 'item') {
                ...
                if (!isset($v->mainsnak->datavalue)) {
                    continue;
                }
                if (!isset($v->mainsnak->datavalue->value)) {
                    continue;
                }
                if ($v->mainsnak->datavalue->value->$nid != str_replace('Q', '', $claim['target'] . '')) {
                    continue;
                }
                $does_exist = true;
                ...
                if ($v->mainsnak->datavalue->value != $claim['text']) {
                    continue;
                }
                $does_exist = true;
                ...
                if (!isset($v->mainsnak->datavalue->value->time)) {
                    continue;
                }
                if ($v->mainsnak->datavalue->value->time != $claim['date']) {
                    continue;
                }
                if ($v->mainsnak->datavalue->value->precision != $claim['prec']) {
                    continue;
                }
                $does_exist = true;
            } else {
                if ($claim['type'] == 'monolingualtext') {
                    ...
                    if (!isset($v->mainsnak->datavalue->value->text)) {
                        continue;
                    }
                    if ($v->mainsnak->datavalue->value->text != $claim['text']) {
                        continue;
                    }
                    if ($v->mainsnak->datavalue->value->language != $claim['language']) {
                        continue;
                    }
                    $does_exist = true;
                } else {
                    if ($claim['type'] == 'quantity') {
                        ...
                        if (!isset($v->mainsnak->datavalue->value->amount)) {
                            continue;
                        }
                        if ($v->mainsnak->datavalue->value->amount != $claim['amount']) {
                            continue;
                        }
                        if ($v->mainsnak->datavalue->value->unit != $claim['unit']) {
                            continue;
                        }
                        $does_exist = true;
                    }
    ...
    function removeClaim($id, $baserev)
    {
        ...
        $this->setToolTag($params, $summary);
        if (isset ($baserev) and $baserev != '') {
            $params['baserevid'] = $baserev;
        }
    ...
    function setSource($statement, $snaks_json)
    {
        ...
        $this->last_res = $res;
        if (isset ($res->error)) {
            if ($res->error->code == 'modification-failed') {
                return true;
            } // Already exists, no real error
    ...
    function genericAction($j, $summary = '')
    {
        ...
        if ($j->action == 'wbeditentity' and isset($j->{'new'})) {
            $this->sleepAfterEdit('create');
        } else {
            $this->sleepAfterEdit('edit');
        }
