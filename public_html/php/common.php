 ...
 * @return mixed|null
 */
function env(string $key, $default = null, $type = null)
{
...
function myurlencode($t)
{
    ...
    return $wrapper_tfc->urlEncode($t);
}
...
function escape_attribute($s)
{
    ...
    return $wrapper_tfc->escapeAttribute($s);
}
...
function myflush()
{
    ...
    return $wrapper_tfc->flush();
}

function get_common_header($script, $title = '', $p = [])
{
    global $wrapper_tfc;

    return $wrapper_tfc->getCommonHeader($title, $p);
}
...
function get_common_footer()
{
    ...
    return $wrapper_tfc->getCommonFooter();
}
...
function openToolDB($dbname = '', $server = '', $force_user = '')
{
    ...
    return $wrapper_tfc->openDBtool($dbname, $server, $force_user);
}
...
function openDBwiki($wiki, $slow_queries = false)
{
    ...
    return $wrapper_tfc->openDBwiki($wiki, $slow_queries);
}
...
function openDB($language, $project, $slow_queries = false)
{
    ...
    return $wrapper_tfc->openDB($language, $project, $slow_queries);
}
...
function getSQL(&$db, &$sql, $max_tries = 2, $message = '')
{
    ...
    return $wrapper_tfc->getSQL($db, $sql, $max_tries, $message);
}
...
function getPagesInCategory($db, $category, $depth = 0, $namespace = 0, $no_redirects = false)
{
    ...
    return $wrapper_tfc->getPagesInCategory($db, $category, $depth, $namespace, $no_redirects);
}
...
function getSPARQL($cmd)
{
    ...
    return $wrapper_tfc->getSPARQL($cmd);
}
...
function getSPARQLitems($cmd, $varname = 'q')
{
    ...
    foreach ($items as $k => $v) {
        $items[$k] = preg_replace('/\D/', '', $v);
    } # Legacy function returns numeric-only, ToolforgeCommon method returns Qxxx, need to rewrite

    return $items;
}

# Misc
function get_server_for_lp($lang, $project)
{
    global $wrapper_tfc;

    return $wrapper_tfc->getWebserverForWiki($wrapper_tfc->getWikiForLanguageProject($lang, $project));
}
...
{
    return "//" . get_server_for_lp($lang, $project) . "/wiki/Special:Redirect/file/" . myurlencode($image);
}

function get_thumbnail_url($lang, $image, $width, $project = "wikipedia")
{
    return "//" . get_server_for_lp($lang, $project) . "/wiki/Special:Redirect/file/" . myurlencode($image) . "?width={$width}";
}
...
function get_wikipedia_url($lang, $title, $action = "", $project = "wikipedia")
{
    ...
    return "https://" . get_server_for_lp($lang, $project) . "/w/index.php?title=" . $wrapper_tfc->urlEncode($title) . ($action == '' ? '' : "&action={$action}");
}
...
function get_db_safe($s, $fixup = false)
{
    global $db;
    if ($fixup) {
        $s = str_replace(' ', '_', trim(ucfirst($s)));
    }
    if (!isset($db)) {
        return addslashes(str_replace(' ', '_', $s));
    }
